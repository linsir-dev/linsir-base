...
import com.linsir.base.core.mapper.DictionaryMapper;
...
public class DictionaryServiceExtImpl extends BaseServiceImpl<DictionaryMapper, Dictionary> implements DictionaryService, DictionaryServiceExtProvider {
    ...
    @Override
    public FilterDataVO conversionFilterDataVO(String title, String key, boolean multiple, List<Dictionary> dictionaryList) {
        FilterDataVO filterDataVO = new FilterDataVO();
        filterDataVO.setKey(key);
        filterDataVO.setMultiple(true);
        filterDataVO.setTitle(title);
        List<Option> optionList = new ArrayList<>();
        dictionaryList.stream().forEach(dictionary -> {
            Option option = new Option();
            option.setLabel(dictionary.getItemName());
            option.setValue(dictionary.getId().toString());
            ...
            for(Dictionary dict : newDictList){
                ...
                if(V.notEmpty(dict.getId())){
                    ...
                    if(!super.updateEntity(dict)){
                        ...
                        throw new BusinessException(BaseCode.FAIL_EXCEPTION, "更新字典子项异常");
                    }
                }
                else{
                    if(!super.createEntity(dict)){
                        log.warn("新建字典子项失败，itemName=" + dict.getItemName());
                        throw new BusinessException(BaseCode.FAIL_EXCEPTION, "新建字典子项异常");
                    }
                }
            }
        }
            ...
            for(Dictionary dict : oldDictList){
                if(!dictItemIds.contains(dict.getId())){
                    if(!super.deleteEntity(dict.getId())){
                        ...
                        throw new BusinessException(BaseCode.FAIL_EXCEPTION, "删除字典子项异常");
                    }
                }
            }
        }
